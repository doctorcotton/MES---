...
**关键点：**
1. **数据模型是权威来源**：所有存储、传输、与MES对接都基于表意数据模型
2. **流程图是派生视图**：从数据模型自动生成，用于帮助理解
3. **显示问题不影响数据**：即使流程图渲染有问题，数据完整性不受影响
4. **编辑方式灵活**：可以用表形界面编辑（直观），也可以用表意界面编辑（精确），两者实时同步到同一数据模型
