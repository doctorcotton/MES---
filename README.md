...
- ⚙️ **工艺类型配置**: 可配置子步骤类型和工艺段类型的默认模板
- 🔧 **动态字段配置**: 支持自定义字段类型、验证规则和UI组件，完全由数据库驱动
- 📅 **设备调度**: 自动计算设备占用时间线，支持依赖关系和设备分配
- 📊 **甘特图视图**: 可视化显示设备调度甘特图，支持研发视图和生产视图切换
...
1. **访问配置页面**: 
   - 在浏览器地址栏输入 `/config` 或通过导航菜单访问
   - 配置页面支持编辑子步骤类型和工艺段类型的默认模板

2. **配置子步骤类型**:
   - 在"子步骤类型配置"标签页中查看所有工艺类型
   - 点击编辑按钮修改默认名称、默认设备、描述等信息
   - 配置会持久化保存到本地存储

3. **配置工艺段类型**:
   - 在"工艺段类型配置"标签页中管理工艺段模板
   - 可以添加、编辑、删除工艺段类型
   - 为每个工艺段配置默认的子步骤序列

4. **字段定义配置**:
   - 在"字段配置"标签页中管理动态表单字段
   - 定义字段类型（文本、数字、数组、对象等）、验证规则和默认值
   - 配置即时生效，驱动 ParamsModal 中的表单渲染

### 设备调度和甘特图

1. **查看甘特图**:
   - 在流程图视图右侧切换到"甘特图"标签
   - 甘特图自动根据配方数据计算设备占用时间线

2. **视图模式切换**:
   - **研发视图**: 使用默认设备池，适合配方研发阶段
   - **生产视图**: 选择具体产线，使用该产线的实际设备配置

3. **甘特图操作**:
   - 使用缩放按钮调整时间轴显示比例
   - 点击占用块可高亮对应步骤并滚动到表格对应行
   - 超过50分钟的任务会标红显示警告
   - 底部显示总耗时和警告信息
